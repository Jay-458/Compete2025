...
class draw_circle():
    ...
    def generate_circle_points(theta):
        center = (14, 9)
        a,b = center
        radius = draw_circle.get_radius(points = np.float32([[0, 0], [27, 0], [27, 18], [0, 18]]), scale_y=0.34)
        x = a + radius * np.cos(theta)
        y = b + radius * np.sin(theta)
        circle_points = (x,y)
    ...
    def transform_and_inverse(src_points,theta):
        ...
        # 5. 在目标坐标系中计算目标点
        transformed_target = draw_circle.generate_circle_points(theta)
        ...
        circle_point = (original_x, original_y)
        return circle_point
if __name__ == "__main__":
    ...
    while True:
        ...
        if center != (0,0):
            i += 1
            circle_points = draw_circle.transform_and_inverse(points,theta)
            x, y = circle_points
            center = (int(x),int(y))
            print(center)
